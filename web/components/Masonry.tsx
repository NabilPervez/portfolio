        ...
        urls.map(
            src =>
                new Promise<void>(resolve => {
                    ...
                    const img = new Image();
                    img.src = src;
                    // If src is invalid or fails, just resolve
...
const Masonry = ({
    ...
    // We need to calculate height of container to set it manually if needed, 
    // but for absolute positioning we rely on parent.
    // Actually, we should calculate the max height of columns to set container height.
    const [containerHeight, setContainerHeight] = useState(0);

    const getInitialPosition = (item: any) => {
        const containerRect = containerRef.current?.getBoundingClientRect();
        if (!containerRect) return { x: 0, y: 0 }; // default fallback
    ...
    const grid = useMemo(() => {
        ...
        const computedGrid = items.map(child => {
            ...
            // Fix: Use provided height or default to 300px.
            // Original code divided by 2, which is oddly specific to a dataset. 
            // We'll trust the height if given, or use a default.
            const rawHeight = child.height || 300;
